...
function cell_ui.render_highlights(bufnr)
  ...
  for _, cell in ipairs(cell_ui.buffer_cells[bufnr]) do
    ...
    -- Hide the actual "# %%" marker line by setting its foreground color to match the background
    ...
    -- Apply subtle background highlighting to the cell content area
    -- For code cells only, and only if configured
    if cell.type == "code" or is_active then
      -- Content area - use subtle highlight
